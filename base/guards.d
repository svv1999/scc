...
bool ms( lazy void cmd, bool delegate()[] guards ...){
  bool lazyAnd= true;
    debug( guards) debug( dbg1) write( "ms[ ");
  foreach( c; guards) {
    if( !(lazyAnd &= c())){
        debug( guards) debug( dbg1) write("*");
      break;
    }
  }
  bool yn= lazyAnd;
...
bool scout( lazy bool exp, lazy void thenAction, lazy void elseAction){
    debug( guards) debug( dbg1) write( "scout[ ");
  bool yn= exp;
  if( yn) {
	   debug( guards) debug( dbg1) write("+");
	 thenAction();
...
bool refuse( lazy bool exp, lazy void refuseAction){
    debug( guards) debug( dbg1) write( "refuse: ");
...
bool teach( lazy bool check, lazy void teachAction, bool checkPost= false){
    debug( guards) debug( dbg1) write( "teach[ ");
...
unittest{
  ...
  ms( {
        ...
        debug( guards) debug( dbg1) writeln( "command");
    }(),
      refuse( i==2, {}()),
      teach( i!=1, i=1)
  );
